...
export function useOpenFileInNeovim(featureId: number): OpenInNeovim | undefined {
  ...
  const openInNeovim = useCallback<OpenInNeovim>(
    (filePath, line, col) => {
      ...
      // whatever list was clicked instead of Neovim. `NeovimPane`'s surface
      // carries the same `aria-label` `useNeovimPointer` already relies on to
      // focus it on a real click; do the same here rather than plumbing a ref
      // through the file tree and every other future caller.
